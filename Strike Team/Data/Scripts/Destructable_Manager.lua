...
function DamageObject(damage, object)
	if object ~= nil and object:IsA("CoreObject") then
		...
		if table ~= nil then
			...
			UpdateObjState(table, obj)
			if (table.respawnTime and damage > 0) then
				Task.Wait(table.respawnTime)
				DamageObject(-damage, obj)
			end
		end
	end
end
...
function UpdateObjState(table, obj)
	...
	if newState ~= table.state then
		...
		if table.state == kStateDestroyed and table.destroyedFx ~= nil then
			...
			-- if they have a ToSpawn, spawn that too
			if (table.toSpawn ~= nil) then
				--[[local offsets = {Vector3.New(40, 40, 40), Vector3.New(-40, 40, 40), Vector3.New(0, -40, 40)}
				for _,o in pairs(offsets) do
					World.SpawnAsset(table.toSpawn, {position = obj:GetWorldPosition() + o})
				end]]--
				World.SpawnAsset(table.toSpawn, {position = obj:GetWorldPosition()})
			end
		end
