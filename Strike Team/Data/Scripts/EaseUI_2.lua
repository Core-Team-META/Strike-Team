...
function Module.EaseValueEx(start, goal, easeDuration, easingEquation, easingDirection, callback, onComplete)
	...
	local task = Task.Spawn(function()
		...
		local newValue = CoreMath.Clamp(easingFormula(currentTime, start, direction * math.abs(goal - start), easeDuration), minValue, maxValue)
		currValue = newValue
...
function Module.Ease(uiElement, property, goal, easeDuration, easingEquation, easingDirection, callback)
	if(type(easeDuration) == "nil") then easeDuration = 1 end
	if(type(easingEquation) == "nil") then easingEquation = Module.EasingEquation.LINEAR end
	if(type(easingDirection) == "nil") then easingDirection = Module.EasingDirection.INOUT end
	...
	wrapTask(property, uiElement, function()
		...
		local oldValue = uiElement[property]
		local newValue = CoreMath.Round(easingFormula(currentTime, start, direction * math.abs(goal - start), easeDuration))
		uiElement[property] = newValue

		if callback then
			callback(uiElement, oldValue, newValue)
		end
	end)
end

function Module.EaseX(uiElement, goal, easeDuration, easingEquation, easingDirection, callback)
	Module.Ease(uiElement, "x", goal, easeDuration, easingEquation, easingDirection, callback)
end

function Module.EaseY(uiElement, goal, easeDuration, easingEquation, easingDirection, callback)
	Module.Ease(uiElement, "y", goal, easeDuration, easingEquation, easingDirection, callback)
end

function Module.EaseWidth(uiElement, goal, easeDuration, easingEquation, easingDirection, callback)
	Module.Ease(uiElement, "width", goal, easeDuration, easingEquation, easingDirection, callback)
end

function Module.EaseHeight(uiElement, goal, easeDuration, easingEquation, easingDirection, callback)
	Module.Ease(uiElement, "height", goal, easeDuration, easingEquation, easingDirection, callback)
end

function Module.EaseRotation(uiElement, goal, easeDuration, easingEquation, easingDirection, callback)
	Module.Ease(uiElement, "rotationAngle", goal, easeDuration, easingEquation, easingDirection, callback)
end

function Module.EaseValue(startValue, endValue, easeDuration, easingEquation, easingDirection, callback, onComplete)
	Module.EaseValueEx(startValue, endValue, easeDuration, easingEquation, easingDirection, callback, onComplete)
end
